    ...
    public class ConfigItems
    {
        ...
        public int GetKeyIndex(string key)
        {
            ...
            for (int i = 0; i < _keyValueList.Count; i++)
            {
                if (_lexer.Substring(_keyValueList[i].Key.StartPos, _keyValueList[i].Key.Length)
                    .Equals(key, System.StringComparison.InvariantCultureIgnoreCase))
                {
                    return i;
                }
            }
        ...
        public string GetValue(string key)
        {
            for (int i = 0; i < _keyValueList.Count; i++)
            {
                if (_lexer.Substring(_keyValueList[i].Key.StartPos, _keyValueList[i].Key.Length)
                    .Equals(key, System.StringComparison.InvariantCultureIgnoreCase)
                    && _keyValueList[i].Value.Type > ConfigTokenType.Null
        ...
        public string GetValue(int keyIndex)
        {
            ...
            if (_keyValueList[keyIndex].Value.Type > ConfigTokenType.Null
